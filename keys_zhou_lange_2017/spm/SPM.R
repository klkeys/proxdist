...
# loss function
f = cmpfun(function(X,S,egs){
    return(- sum(log(egs)) + sum(diag(S%*%X)))
})

test.spm = cmpfun(function(){
    # shore up memory
    rm(list = ls())
    ...
    thr = 1e-6 
    s   = 0.01      # noise level in precision matrix
#    N   = 2^(3:9)
    N   = 2^3
    p   = 1000
    ...
    for (i in 1:length(N)){
        ...
        # vectors to save information for SPM routine in Julia
...
#		glasso.time = proc.time()
            rhos = paths[[i]]
            a = glassopath(s2, rholist = rhos, penalize.diagonal = 0, trace = 0, thr = thr)
#		glasso.time = proc.time() - glasso.time
            ...
            for (j in 1:length(rhos)){
                ...
                temp     = abs(triu(Z, k = 1))
                rho.to.k = floor(sum(as.numeric(temp > thr)))
                ...
                if(rho.to.k == total.k){
                    ...
                    glasso.time = proc.time()
                    aa = glasso(s = s2, rho = rho, thr = thr, penalize.diagonal = FALSE,
...
#				cat(paste("total.k = ", total.k, "rho.to.k = ", rho.to.k, "\n", sep = ""))
                    if(total.k == rho.to.k){
                        ...
                        k        = k + sum(as.numeric(the.diag > thr))
                        ...
                        write.table(format(Z, digits = 12), paste("./W_", n, "_", rho*10000000, "_", multiruns,".txt", sep = ""), row.names = F, col.names = F, quote = F)
                        write.table(format(s2, digits = 12), paste("./X_", n, "_", multiruns, ".txt", sep = ""), row.names = F, col.names = F, quote = F)
                        rm(Z,s2)
...
#        write.table(format(glasso.times, digits = 12), paste("./glassotime_", n, ".txt", sep = ""), row.names = F, col.names = F, quote = F)
#        write.table(format(losses, digits = 12), paste("./losses_", n,".txt", sep = ""), row.names = F, col.names = F, quote = F)
#        write.table(format(K, digits = 12), paste("./K_", n,".txt", sep = ""), row.names = F, col.names = F, quote = F)
#        write.table(format(band.K, digits = 12), paste("./bandK_", n, ".txt", sep = ""), row.names = F, col.names = F, quote = F)
#        write.table(format(rhos.to.ks, digits = 12), paste("./rhos_", n,".txt", sep = ""), row.names = F, col.names = F, quote = F)
