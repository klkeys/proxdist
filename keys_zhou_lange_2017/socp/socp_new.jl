...
function proj_soc!(
    #xp :: Union{SparseMatrixCSC{T,Int}, DenseVector{T}},
    #x  :: Union{SparseMatrixCSC{T,Int}, DenseVector{T}},
    xp :: AbstractVector{T},
    x  :: AbstractVector{T},
    r  :: T 
...
#    (p,q) == size(A) || throw(DimensionMismatch("nonconformable A, b, and c"))
    ...
    for i = 1:max_iter
        ...
        #ky = one(T) + kx
        #U .= ky*u .+ kx .* u0
        compute_accelerated_step!(U, u0, u, i)
...
#        pw, pr = proj_soc(W,R)
        ...
        invrho = one(T)/rho
         u = (invrho*I + AA) \ (invrho*x + A'*(pw - b) + (pr - d)*c)
        #prox_quad!(u, V, ev, z, x, rho, y2=y2, n=q)
        prox_quad!(u, y2, V, ev, z, x, invrho)
...
function proj_soc(
    ...
    u        :: SparseMatrixCSC{T,Int} = sprandn(q, 1, 0.1),
    v2       :: SparseMatrixCSC{T,Int} = zeros(T, p), 
    U        :: SparseMatrixCSC{T,Int} = copy(u), 
    u0       :: SparseMatrixCSC{T,Int} = copy(u),
    w        :: SparseMatrixCSC{T,Int} = spzeros(T, p, 1),
    W        :: SparseMatrixCSC{T,Int} = copy(w), 
    pw       :: SparseMatrixCSC{T,Int} = copy(b),
    z        :: SparseMatrixCSC{T,Int} = spzeros(T, q, 1),
    r        :: T = zero(T),
    pr       :: T = r,
    R        :: T = r,
) where {T <: AbstractFloat}
...
function proj_soc(
    ...
    q        :: Int     = length(c), 
    At       :: SparseMatrixCSC{T,Int} = A', 
    u        :: DenseVector{T} = zeros(T, q), 
    v2       :: DenseVector{T} = zeros(T, p), 
    U        :: DenseVector{T} = zeros(T, q), 
    u0       :: DenseVector{T} = zeros(T, q), 
    w        :: DenseVector{T} = zeros(T, p), 
    W        :: DenseVector{T} = zeros(T, p), 
    pw       :: DenseVector{T} = zeros(T, p), 
    z        :: DenseVector{T} = zeros(T, q), 
    b0       :: DenseVector{T} = zeros(T, q), 
    r        :: T = zero(T),
    pr       :: T = r,
    R        :: T = r,
) where {T <: AbstractFloat}
